...
```powershell
$project = if ($env:BRIEFBOUND_HUAWEI_NSLB_ROOT) {
  (Resolve-Path -LiteralPath $env:BRIEFBOUND_HUAWEI_NSLB_ROOT).Path
} else {
  (git rev-parse --show-toplevel 2>$null)
}
if (-not $project -or -not (Test-Path -LiteralPath (Join-Path $project "src\Solution.cpp"))) {
  throw "请通过 --project、BRIEFBOUND_HUAWEI_NSLB_ROOT 或当前仓库提供 Huawei NSLB 项目根目录。"
}
$codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $HOME ".codex" }
$tool = Join-Path $codexHome "skills\briefbound-huawei-nslb-score-loop\scripts\score_loop_tools.py"
python $tool --help
```
