...
Always generate the HTML first — `svg` and `png` are produced *from* it via [`delivery.md`](delivery.md). Never hand-author an SVG file directly; the HTML is the source of truth and the only artifact the taste gate is written against.
...
- [ ] Requested non-HTML formats produced via [`delivery.md`](delivery.md), not hand-authored?
