...
This is a **second, fixed skin** — see [style-guide.md § Terminal skin](style-guide.md#terminal-skin-opt-in-alternate) for the token table. It does not inherit from the task-local brand adaptation described in [branding.md](branding.md) and isn't part of the light/dark inversion rule; every terminal example uses the same nine tokens regardless of the host site's brand.
...
- Brand-matched output governed by [branding.md](branding.md) — terminal is a fixed skin, not brand-tokenized. Don't try to reconcile the two.
