...
- 用户可见内容默认中文，保留代码、命令、路径、错误原文、API/协议、skill 名和枚举；Route Out 仅以 Briefbound task contract 为准，末行写 `下一步建议: <一个具体动作>`。
...
没有 finding 时说明审查通过，并列出未验证的状态、视口或运行环境。末行：`下一步建议: <一个具体动作>`。
