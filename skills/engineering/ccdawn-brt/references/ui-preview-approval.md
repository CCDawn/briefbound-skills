...
- 信息架构、任务流、交互和响应式结果由 `ccdawn-ui-design` 生成预览。
- 品牌、字体、色彩、构图、图像、图标和动效方向由 `ccdawn-visual-design` 生成预览。
- `ccdawn-ui-review` 只读检查预览，输出 `READY_FOR_USER_APPROVAL` 或 `REVISE_RECOMMENDED`；它不能代表用户批准。
- `ccdawn-frontend-engineering` 只消费 `APPROVED` 或有依据的 `PREVIEW_SKIPPED` 契约。缺批准证据且命中触发条件时，回到对应设计 owner。
