...
- Route Out: 预览无阻塞 finding 时交用户批准；需要调整时回原设计 owner；用户已批准的独立实现转 `ccdawn-frontend-engineering`；系统性 token、主题或组件问题转 `ccdawn-design-system`；整份 PR 正确性由 `ccdawn-pr-review` 主责；阻塞回 `ccdawn-brt`。
...
这两个结果都不是 `APPROVED`。只有用户对具体预览明确批准，BRT 才能允许正式 UI 写入。
