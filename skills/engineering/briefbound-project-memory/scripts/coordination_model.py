...
def coordination_root(project_root: Path, codex_home: Path | None = None) -> Path:
    ...
    if common_dir is not None:
        current_root = common_dir / "briefbound" / "coordination"
        legacy_root = common_dir / "ccdawn" / "coordination"
        current_registry = current_root / "registry.json"
        legacy_registry = legacy_root / "registry.json"
        if current_registry.exists() and legacy_registry.exists():
            raise CoordinationConflict(
                "Both Briefbound and legacy coordination registries exist; reconcile them before continuing."
            )
