...
Load this file when the user points at a `.drawio`, `.drawio.xml`, `.drawio.png`, or `.drawio.svg` file and wants a diagram out of it — "convert this drawio", "redraw this diagram", "make this presentable", "この drawio をきれいにして", or explicitly invokes `$briefbound-diagram-design`.
...
`<skill-dir>` is `` in this repo, or the skill's own directory when it's installed standalone or as a plugin. If the path isn't obvious, glob for `**/diagram-design/scripts/drawio_extract.py`.
...
3. Produce `svg` / `png` if the format dial asked for them — via [`delivery.md`](delivery.md), from the HTML.
