...
### Upstream planning guidance (Briefbound entrypoint governs approval)
...
- [ ] **No label mask overlaps a node drawn after it? (Node fill would clip the text — §6 rule 6. Run `python3 scripts/verify-geometry.py <file>` from the skill root.)**
...
- [ ] If animated, does the complete static/no-JS frame work, does reduced motion hide/disable playback, and is the controller copied verbatim from `assets/template-motion.html`? Run `self_check.py` and `verify-motion.py`, then manually check print and static-query states.
