...
Default skin is a cool editorial palette — white-smoke paper, jet-black ink, atomic-tangerine accent, blue-slate muted. It's designed to look good out of the box; copy this file into the task and adapt it through [`branding.md`](branding.md) when a brand match is required.

To derive task-local tokens from a website or project, see [`branding.md`](branding.md).
...
1. **Adapt a task copy** — see [`branding.md`](branding.md). Sample the authorized source, propose the palette and fonts, then write the task-local copy.
...
4. **Reusable project tokens** — use the project's existing design-system source rather than a hidden global profile.
