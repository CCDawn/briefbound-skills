...
- 发散过程中支持可选交流碰撞环节。默认跳过；只有用户明确开启、创意质量明显依赖多角色碰撞，或 BRT 路由为深度共创时才询问或开启。
...
7. 默认跳过交流碰撞；如果用户确认、明确要求或 BRT 判断深度共创有价值，运行 Collision Forum。
...
- 在生成“原始发散”之后，默认跳过交流碰撞；只有用户明确开启、创意质量明显依赖多角色碰撞，或 BRT 路由为深度共创时才询问或开启。
...
```text
如果需要深度共创，询问：是否开启“角色交流碰撞”？开启后，本轮抽取的顶尖角色会互相质询、反驳、补强，并产出碰撞后的二代想法。回复“开启”或“跳过”即可。
```
