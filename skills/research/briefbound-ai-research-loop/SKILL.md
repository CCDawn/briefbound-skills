...
4. 可在当前上下文完成的简单比较直接执行；反复晋升、leaderboard 或持久 lane 才交给 `briefbound-score-loop`。
...
重要 baseline 晋升、论文级结论或反直觉结果先进入 `briefbound-research-rigor-review`。普通失败 lane 不增加正式审查。
