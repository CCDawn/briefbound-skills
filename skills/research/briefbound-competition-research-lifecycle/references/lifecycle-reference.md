...
这份 reference 定义了面向竞赛型科研工作的阶段模型、证据账本、逐阶段 Briefbound gates、checklists、并行 lane 和路由规则。
...
这里的语言框架是 Briefbound Router，而不是额外再加一个独立业务步骤。
...
Routing：
- 用 `briefbound-router` 锁定范围和阶段退出标准
- 如果领域背景、历史 baselines 或 policy context 还不清楚，使用当前可用的浏览/研究工具收集来源，再交给 `briefbound-ai-research-loop` 收敛
...
- `briefbound-ai-research-loop`：把来源、baseline 和假设标准化为可验证的研究 lane
...
- `briefbound-ai-research-loop`：当 related work、方法定位或实验方向仍然松散时使用
- `briefbound-research-rigor-review`：当 draft、结构化 outline 或重要 baseline claim 已存在时使用
