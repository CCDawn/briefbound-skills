...
- Before the first write, scope expansion, or merge, run the project coordination `preflight` with `development`, `mechanical`, or `integration` intent when that command is available. Ordinary development on the primary `main/master` checkout must move to a task worktree; mechanical writes require explicit classification; integration requires the matching active claim and a clean target.
