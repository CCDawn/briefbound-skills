...
SKILL_READ_RE = re.compile(
    r"(?:^|[\\/]+)(?P<name>briefbound-[a-z0-9-]+)[\\/]+SKILL\.md",
    re.IGNORECASE,
)
SKILL_NAME_RE = re.compile(
    r"^\s*name:\s*[\"']?(?P<name>briefbound-[a-z0-9-]+)[\"']?\s*$",
...
def run_case(
    ...
    if artifact_root is None:
        temp_context = tempfile.TemporaryDirectory(prefix=f"briefbound-router-{case['id']}-")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run live, read-only Briefbound Router checks.")
    ...
    parser.add_argument("--allow-inactive", action="store_true", help="Allow a baseline run without the managed Briefbound Router activation block.")
...
def main() -> int:
    ...
    if "<!-- Briefbound Router activation: start -->" not in activation_text and not args.allow_inactive:
        raise SystemExit(
            "Briefbound Router global activation is absent. Run install.ps1 first or pass --allow-inactive for a baseline."
        )
